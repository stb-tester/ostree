...
ostree_builtin_commit (int argc, char **argv, OstreeCommandInvocation *invocation, GCancellable *cancellable, GError **error)
{
  ...
  if (argc <= 1 && (opt_trees == NULL || opt_trees[0] == NULL))
    {
      if (!ostree_repo_write_dfd_to_mtree (repo, AT_FDCWD, ".", target_mtree, modifier,
                                           cancellable, error))
        goto out;
    }
      ...
      for (tree_iter = (const char *const*)opt_trees; *tree_iter; tree_iter++)
        {
          ...
          if (strcmp (tree_type, "dir") == 0)
          ...
          else if (strcmp (tree_type, "ref") == 0)
            {
              ...
              if (!ostree_repo_write_directory_to_mtree (repo, (GFile*) tree_in,
                                                         target_mtree, modifier,
                                                         cancellable, error))
                goto out;
            }
    ...
    {
      g_assert (argc > 1);
      if (!ostree_repo_write_dfd_to_mtree (repo, AT_FDCWD, argv[1],
                                           target_mtree, modifier, cancellable,
                                           error))
        goto out;
    }
